...
// Lib
import { CacheContext, newEventDrivenCache, useEventDrivenCache } from '../lib/cache'
...
const decodeMessage = (message: WithPayload<MessageV0>): ProfileProto | false => {
...
const updateProfile = async (
	...
	const pictureHash = profile.avatar ? await postPicture(waku, profile.avatar) : undefined
...
export const useSyncProfile = () => {
	...
	const { profile: wakuProfile, message, loading } = useProfile(profile?.address ?? '')
	...
	useEffect(() => {
		...
		if (address !== profile.address) {
			console.error(`Profile address (${profile.address}) differs from signer address (${address})`)
			return
		}
		...
		else if (message && Date.now() - (profile.lastSync?.getTime() ?? 0) > 24 * 60 * 60 * 1000) {
