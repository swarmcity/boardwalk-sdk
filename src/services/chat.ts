...
// Services
import { useWakuFilter, useWakuStoreQuery, WithPayload, wrapFilterCallback } from './waku'
...
export const deriveChatPrivateKey = async (
	...
	const base = await crypto.subtle.importKey('raw', baseKey, { name: 'HKDF' }, false, [
		'deriveKey',
		'deriveBits',
	])
...
const useChatKeys = (marketplace: string, item: bigint) => {
	...
	return useAsync(async () => formatChatKeys(marketplace, item, chatKeys), [chatKeys])
}
...
export const selectTempChatKey = (marketplace: string, item: bigint, address: string) => {
	setStore.keys[getRecordKey(marketplace, item)]((keys) => {
		if (!keys) {
			return keys
		}
...
const decodeWakuMessage = async (message: WithPayload<MessageV1>): Promise<ChatMessage> => {
	return {
...
export const postChatMessage = async (
	...
	await waku.lightPush.push(
		new SymEncoder(getChatMessageTopic(marketplace, item), keys.symKey, keys.mySigPrivKey),
		{ payload },
	)
}
