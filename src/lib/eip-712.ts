...
// Types
import type { TypedDataDomain, TypedDataField, Signer } from '@ethersproject/abstract-signer'
...
const getVerifyPayloadConfig = <ProtoType>(config: VerifyPayloadConfig<ProtoType>) => {
	return {
		...config,
...
export const verifyPayload = <ProtoType extends Record<string, unknown>>(
	...
	const { getSigner, formatValue, getSignature } = getVerifyPayloadConfig(config)
...
>(
	...
	try {
		...
		const eipConfig = typeof eip712Config === 'function' ? eip712Config(decoded) : eip712Config
...
>(
	...
	// Sign the message
	const signatureHex = await signer._signTypedData(config.domain, config.types, values)
...
>(
	...
	const payload = await createSignedPayload(config, formatData, formatDataToSign, signer)
